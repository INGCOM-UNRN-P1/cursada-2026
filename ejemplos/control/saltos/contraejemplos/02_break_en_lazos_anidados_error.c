 ...
 * EXPLICACION: 'break' solo interrumpe el lazo mas interno que lo contiene.No
 * sale de los lazos envolventes superiores.
 */
...
int main(void)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        {
            if (j == 1)
                break; // Sale del lazo j, pero el lazo i continua iterando
            printf("  i=%d, j=%d\n", i, j);
        }
